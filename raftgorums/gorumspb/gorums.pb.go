...
type internalAppendEntriesResponse struct {
	nid   uint32
	reply *raftpb.AppendEntriesResponse
	err   error
}

type internalRequestVoteResponse struct {
	nid   uint32
...
/* Unexported quorum call method AppendEntries */
func (c *Configuration) appendEntries(ctx context.Context, a *raftpb.AppendEntriesRequest, f func(arg raftpb.AppendEntriesRequest, nodeID uint32) *raftpb.AppendEntriesRequest) (resp *raftpb.AppendEntriesQFResponse, err error) {
	var ti traceInfo
	if c.mgr.opts.trace {
		ti.Trace = trace.New("gorums."+c.tstring()+".Sent", "AppendEntries")
		defer ti.Finish()
	...
	expected := c.n
	replyChan := make(chan internalAppendEntriesResponse, expected)
	for _, n := range c.nodes {
		...
		go callGRPCAppendEntries(ctx, n, nodeArg, replyChan)
	}

	var (
		replyValues = make([]*raftpb.AppendEntriesResponse, 0, expected)
		errCount    int
		quorum      bool
	)
...
func callGRPCAppendEntries(ctx context.Context, node *Node, arg *raftpb.AppendEntriesRequest, replyChan chan<- internalAppendEntriesResponse) {
	...
	replyChan <- internalAppendEntriesResponse{node.id, reply, err}
}
...
/* Unexported quorum call method RequestVote */
func (c *Configuration) requestVote(ctx context.Context, a *raftpb.RequestVoteRequest) (resp *raftpb.RequestVoteResponse, err error) {
	var ti traceInfo
	if c.mgr.opts.trace {
		ti.Trace = trace.New("gorums."+c.tstring()+".Sent", "RequestVote")
		defer ti.Finish()
	...
	expected := c.n
	replyChan := make(chan internalRequestVoteResponse, expected)
	for _, n := range c.nodes {
		go callGRPCRequestVote(ctx, n, a, replyChan)
	}

	var (
		replyValues = make([]*raftpb.RequestVoteResponse, 0, expected)
		errCount    int
		quorum      bool
	)
...
func callGRPCRequestVote(ctx context.Context, node *Node, arg *raftpb.RequestVoteRequest, replyChan chan<- internalRequestVoteResponse) {
	...
	replyChan <- internalRequestVoteResponse{node.id, reply, err}
}
...
var fileDescriptorGorums = []byte{
	...
	0x30, 0xd7, 0xf3, 0xc2, 0x64, 0x5c, 0x73, 0x0b, 0x4a, 0x2a, 0x9d, 0x54, 0x2e, 0x3c, 0x94, 0x63,
	...
	0xcb, 0x31, 0x24, 0xb1, 0x81, 0xe3, 0xc0, 0x18, 0x10, 0x00, 0x00, 0xff, 0xff, 0x99, 0x40, 0xd2,
	0xc6, 0x29, 0x02, 0x00, 0x00,
}
